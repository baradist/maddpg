...
class Scenario(BaseScenario):
    ...
    def reward(self, agent, world):
        if agent.goal_a is None or agent.goal_b is None:
            return 0.0
        dist2 = np.sum(np.square(agent.state.p_pos - agent.goal_a.goal_b.state.p_pos))
        is_close = np.square(agent.size + agent.goal_a.goal_b.size) >= dist2
        if is_close:
            return 10
        return -dist2
    ...
    def observation(self, agent, world):
        ...
        goal_color[0] = agent.goal_a.goal_b.color
        ...
        entity_pos_ = []
        for entity in world.landmarks:
            entity_pos_.append(entity.state.p_pos - agent.goal_a.state.p_pos)
        # entity colors
        entity_color = []
        for entity in world.landmarks:
